...
export const like = async ({ postId, token }) => {
  try {
    await axios.post(`${BASE_URL}/like/like`, { postId }, { headers: { Authorization: `Bearer ${token}` } });
  } catch (error) {
    console.log(error);
  }
};
...
export const unLike = async ({ postId, token }) => {
  try {
    const response = await axios.post(`${BASE_URL}/like/unlike`, { postId }, { headers: { Authorization: `Bearer ${token}` } });
...
export const fetchLikes = async ({ postId, setLikeCount, setIsLiked, userId }) => {
  try {
    ...
    response.data.likeDocs.forEach((likedoc) => {
      if (likedoc.userId == userId) setIsLiked(true);
    });
  } catch (error) {}
};

export const fetchReposts = async ({ postId, setRepostCount, setIsRepost, userId }) => {
  try {
    const response = await axios.post(`${BASE_URL}/post/reposts`, { postId });
    setRepostCount(response.data.length);
    response.data.forEach((repostdoc) => {
      if (repostdoc.reposterId == userId) setIsRepost(true);
    });
  } catch (error) {}
};

export async function handleRepost({ post, reposterId, token, user, dispatch }) {
  try {
    const response = await axios.post(`${BASE_URL}/post/create`, {
      userId: post.userId._id, isRepost: true, reposterId, originalPostId: post._id, title: post.title, image: post.image,
...
export async function handleUndoRepost({ post, token, dispatch, user }) {
  try {
    const response = await axios.post(`${BASE_URL}/post/undorepost`, { postId: post._id }, { headers: { Authorization: `Bearer ${token}` } });
    console.log(response);
...
export const handleEditPost = async ({ postId, title, file, token, image, setIsSaving, dispatch }) => {
  setIsSaving(true);
  try {
    ...
    const response = await axios.post(`${BASE_URL}/post/edit`, { postId, data: body }, { headers: { Authorization: `Bearer ${token}` } });
