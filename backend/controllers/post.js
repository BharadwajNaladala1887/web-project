...
import { createPost, deletePost, editPost, findPostById, getPostById, getPosts, getPublicPosts, getReposts, getUserPosts } from "../models/Post.js";
...
export const handleCreatePost = async (req, res) => {
  try {
    ...
    let postDoc;
    if (!isRepost) {
      postDoc = await createPost({ userId, title, image });
    } else {
      postDoc = await createPost({ userId: req.body.userId, isRepost, reposterId, originalPostId, title, image });
    }
...
// Handles fetching all public posts
...
export const handlePostDelete = async (req, res) => {
  try {
    ...
    if (!post.userId.equals(req.user._id)) return res.status(401).json({ message: "Unauthorized request" });
...
// Handles undoing a repost
export const handleUndoRepost = async (req, res) => {
  try {
    const postId = req.body.postId;
    const post = await findPostById({ postId });
    if (!post) return res.status(400).json({ message: "No post found." });
    const deletedPost = await Post.findOneAndDelete({ reposterId: req.user._id, originalPostId: post._id });
