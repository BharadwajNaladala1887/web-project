...
// Register a new user
export const registerUser = ({ name, email, username, password, picturePath, location, occupation }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const user = new User({ name, email, username, password, picturePath, location, occupation });
...
export const editProfile = ({ id, newName, newEmail, newUsername, newBio, newLocation, newPicturepath, newBannerpath, newOccupation }) => {
  return new Promise(async (resolve, reject) => {
    try {
...
export const getNewUsers = (count) => {
  return new Promise(async (resolve, reject) => {
    try {
      const users = await User.find().sort({ createdAt: -1 }).limit(count);
      resolve(users);
    } catch (error) {
      reject(error);
    }
